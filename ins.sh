...
# Public IP (taken from local interfaces): prioritize wlan1, then wlan0, then rmnet_data3
public_ip=""
public_label=""

# Check for wlan1
public_ip=$(ifconfig wlan1 2>/dev/null | grep 'inet ' | awk '{print $2}')
if [ -n "$public_ip" ]; then
  public_label="wlan1"
else
  # Check for wlan0 if wlan1 not found
  public_ip=$(ifconfig wlan0 2>/dev/null | grep 'inet ' | awk '{print $2}')
  if [ -n "$public_ip" ]; then
    public_label="wlan0"
  else
    # Check for rmnet_data3 if neither wlan interface is found
    public_ip=$(ifconfig rmnet_data3 2>/dev/null | grep 'inet ' | awk '{print $2}')
    if [ -n "$public_ip" ]; then
...
# Public IP from local interface
public_ip=$(ip -4 addr show wlan0 2>/dev/null | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
public_label="wlan0"

# Fallback if nothing found
[ -z "$public_ip" ] && { public_ip="Not found"; public_label="N/A"; }

wlan_ip=$(python3 - << 'INNER_EOF'
import socket
import fcntl
import struct
